 ...
 * @author Team 28
 * 
 *         The UserAndManagerTerminal is a Thread which communicates with the
 *         user using text through the standard input and standard output. It
 *         lets the person interacting with the program log in or create an
 *         account, and it goes into a loop asking him for the next action. When
 *         the program starts, it also loads the LocalLibraryData from a text
 *         file if avaiable and allows the person (user or manager) to save it
 *         to the text file any time.
 *
 */
public class UserAndManagerTerminal extends Thread {
	...
	/*
	 * NOTE: when testing make all passwords "testpassword". First username should
	 * be "test"
	 */
	...
	try (Scanner scanner = new Scanner(System.in)) {
		...
		System.out.println(
			"Currently there are no manager (librarian) accounts. Please create a manager account.");
	    ...
	    do {
		userOrManagerCommandResult = askAndDoNextUserOrManagerCommand(scanner, localLibraryData,
			currentAccount);
	...
	while (username == null) {
	    ...
	    } else if (localLibraryData.managerAccounts.containsKey(usernameEntered)
		    && localLibraryData.userAccounts.containsKey(usernameEntered)) {
	...
	while (password == null) {
		...
		System.out.println(
			"The password must contain only letters, numbers, _-`~!@#$%^&*()=+[{]}\\|;:'\",<.>/? symbols, or spaces.");
    ...
    private UserOrManagerCommandResult askAndDoNextUserOrManagerCommand(Scanner scanner,
	    LocalLibraryData localLibraryData, Account currentAccount) {
	System.out.println("Welcome " + currentAccount.getUsername()
		+ ". What would you like to do? Enter a number to make a selection.");
	...
	try {
		...
		Files.createDirectories(Paths.get("cosc310_T28_library_system_saved_files/")); // does not overwrite
											       // anyways
	    }
	} catch (IOException e) {
	    e.printStackTrace();
	}
	try (FileOutputStream fileOut = new FileOutputStream("cosc310_T28_library_system_saved_files/main.ser");
//		GZIPOutputStream zipOut = new GZIPOutputStream(fileOut);
	...
	if (Files.exists(Paths.get("cosc310_T28_library_system_saved_files/main.ser"))) {
	    try (FileInputStream fileIn = new FileInputStream("cosc310_T28_library_system_saved_files/main.ser");
